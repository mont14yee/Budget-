...
const Header: React.FC<HeaderProps> = ({ activeView, userProfile, onSelectFeature, setActiveView }) => {
        ...
        return (
            <header className="pt-12 pb-6 px-6 relative z-20 bg-transparent flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-white/50 shadow-md bg-gray-200 flex items-center justify-center">
                        {userProfile?.avatar ? (
                ...
                <div className="flex items-center gap-3">
                    <button className="px-4 py-1.5 rounded-xl border border-cyan-400 text-cyan-500 text-sm font-medium tracking-wide hover:bg-cyan-50 dark:hover:bg-cyan-900/30 transition-colors">
                        Add Card
                    </button>
                    <button aria-label="Settings" 
                        onClick={() => setActiveView?.(ViewType.Settings)}
                        className="w-9 h-9 rounded-xl border border-gray-200 dark:border-gray-700/50 flex items-center justify-center text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                    >
    ...
    return (
        <header className="pt-12 pb-6 px-6 relative z-20 bg-transparent flex items-center justify-between">
            <button aria-label="Back" className="w-9 h-9 rounded-full flex items-center justify-center text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" onClick={() => setActiveView?.(ViewType.Dashboard)}>
